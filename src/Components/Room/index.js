    ...
    <>
      {data.images.map((image, index) => (
        <div
          ...
          key={index}
          className={`card ${selected ? `card-active` : ``} flex flex-row pl-0`}
        >
          <img
          ...
          >
            ...
            <div className="d-flex align-items-end justify-content-between">
              ...
              <div>People: {overview.content.length * 2}</div>
              <div className="price">
                {discount > 0 && (
                  <>
                    <div className="font-weight-normal text-right">
                      <small>
                        € <del>{(data.roomId / 1000000).toFixed(2)}</del>
                      </small>
                    </div>
                    <div>
                      €
                      {(
                        data.roomId / 1000000 -
                        ((discount / 100) * data.roomId) / 1000000
                      ).toFixed(2)}
                    </div>
                  </>
                )}
                {!discount && (
                  <span>€ {(data.roomId / 1000000).toFixed(2)}</span>
                )}
              </div>
            </div>
            <button
